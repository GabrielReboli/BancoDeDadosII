...
CREATE VIEW ClientesContasCidadesDiferentes AS
...
SELECT individual.fname AS Nome,
       individual.lname AS Sobrenome,
       customer.city AS Cidade_Estabelecimento,
       branch.city AS Cidade_Conta
FROM individual
JOIN customer ON individual.cust_id = customer.cust_id
JOIN ClientesContasCidadesDiferentes AS CCD ON customer.cust_id = CCD.cust_id
JOIN account ON customer.cust_id = account.cust_id
JOIN branch ON account.open_branch_id = branch.branch_id
GROUP BY individual.fname, individual.lname, customer.city, branch.city;
...
SELECT open_branch_id, MAX(avail_balance) maximo
    FROM account
    GROUP BY open_branch_id;
...
LEFT JOIN individual ON account.cust_id = individual.cust_id
LEFT JOIN business ON account.cust_id = business.cust_id
JOIN ContaMaiorSaldoPorAgencia ON account.open_branch_id = ContaMaiorSaldoPorAgencia.open_branch_id
AND account.avail_balance = ContaMaiorSaldoPorAgencia.maximo;